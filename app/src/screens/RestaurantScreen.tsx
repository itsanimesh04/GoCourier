...
const RestaurantListHeader = memo(function RestaurantListHeader({
  restaurant,
  filteredCount,
  categories,
  activeCategory,
    ...
    <View>
      ...
      <View className="px-4 pt-5">
        <View className="mb-4 flex-row items-center justify-between">
          <Text className="font-sans text-sm text-muted">{filteredCount} items</Text>
          <Pressable
...
export default function RestaurantScreen() {
  ...
  const sections = useMemo(() => {
    ...
    return cats
      ...
      .map((cat) => ({
        title: cat,
        data: chunkPairs(grouped[cat] ?? []).map((items, i) => ({
          key: `${cat}-${items[0]?.id ?? i}`,
          items,
        })),
      }));
  ...
  }, [restaurant, filtered.length, categories, activeCategory]);
  ...
  const renderItem = useCallback(({ item }: { item: { key: string; items: MenuItem[] } }) => {
    return (
      <View className="mb-3 px-4 flex-row items-start gap-3">
        {item.items.map((food) => (
